...
constructor(nowCount,dispCount,dispChar,per,interval){
        this.nowCount = 0;
        this.dispCount = 0;
        this.dispChar = ["|", "/", "—", "＼"];
        this.per = per;
...
class Branch{
    ...
    this.apiData = require('../imagedata/api').showdata;
    ...
    rl.question(cyan(`please username🪴\nUserName:`),(Name)=>{
        rl.question(cyan(`next password🪴\nLoginPassward:`),(PassWord)=>{
             rl.question(cyan(`last enail_adress🪴\nadress:`),(MailAdress)=>{
                ...
                }).then(([result, created]) => { // thenメソッドを追加
...
MainPrompt(query){
   ...
   rl.question(cyan(`branch:🪴  >>:`), query => {
    ...
    {
        const RunserverWhile = new Progress(this.nowCount,this.dispCount,this.dispChar,this.per, this.interval);
        console.log(yellow(`---run server now🪴  CONTROl-BREAK  http://127.0.0.1:4000/`));
        RunserverWhile.ProgressBerRotate(100,10); //プログレスバー
